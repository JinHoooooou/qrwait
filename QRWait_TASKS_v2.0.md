...
- [x] 매장 상태가 OPEN이 아닌 경우 웨이팅 등록 폼 대신 상태 메시지 표시
    - BREAK: "현재 브레이크타임입니다"
    - FULL: "현재 만석입니다"
    - CLOSED: "오늘 영업이 종료되었습니다"
- [x] SSE로 매장 상태 변경 시 실시간으로 메시지 전환 (`/api/stores/{storeId}/stream` — Phase 4 완료 후 동작)
...
- [ ] 점주 호출 → 호출된 손님 화면만 호출됨 페이지로 이동 확인 (등록완료/실시간 화면 모두, 새로고침 유지, 다른 대기 손님 화면엔 미이동)
...
- [x] 점주가 브레이크타임 설정 후 손님이 QR 스캔 → 등록 불가 메시지 표시 확인
- [x] Access Token 만료 후 자동 재발급 동작 확인
- [x] 타 점주 매장 API 직접 호출 → 404 확인 (리소스 존재 자체를 숨기는 보안 패턴으로 404가 올바른 동작)
- [x] 대기자 수 임계값 초과 시 점주 알림 발송 확인

### 6-3. 예상 대기시간 계산 검증

> ⏱ 15m | 선행: 6-1

- [x] 테이블 수 5개, 평균 이용시간 30분, 앞 대기 3팀 → 손님 화면에 18분 표시 확인
- [x] 점주가 설정 변경 후 손님 화면 예상 대기시간 즉시 반영 확인

### 6-4. 보안 검증

> ⏱ 15m | 선행: Phase 3 완료

- [x] JWT 없이 `/api/owner/**` 접근 → 401 반환 확인
- [x] 만료된 Access Token으로 접근 → 401 반환 확인
- [x] 손님 API (`/api/stores/**`, `/api/waitings/**`) JWT 없이 정상 동작 확인
