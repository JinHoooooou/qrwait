...
- [ ] `LandingPage` 렌더링 테스트 (Vitest + Testing Library)
    - 폼 렌더링 확인
    - 등록 버튼 클릭 시 API 호출 확인 (MSW mock)
...
- [ ] 존재하지 않는 QR코드로 접속 → 에러 메시지 표시 확인
- [ ] 같은 기기에서 중복 등록 시도 → 기존 웨이팅 페이지로 리다이렉트 확인
- [ ] 웨이팅 취소 후 해당 waitingId로 접속 → 안내 메시지 표시 확인
- [ ] SSE 연결 중 서버 재시작 → 자동 재연결 동작 확인
